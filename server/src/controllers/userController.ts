...
// Express Request tipini req.user.id'yi destekleyecek şekilde genişletiyoruz
interface AuthRequest extends Request {
  user?: { 
    userId: number; // Token içinde 'userId' olarak geldiği için burayı düzelttik
    role?: string;
    iat?: number;
    exp?: number;
  };
  ...
  try {
    ...
    if (currentUser.avatarUrl && currentUser.avatarUrl.includes("cloudinary")) {
      try {
        ...
        console.error(
          "Eski resim silinirken hata oluştu (Devam ediliyor):",
          deleteError
        );
      }
    }
